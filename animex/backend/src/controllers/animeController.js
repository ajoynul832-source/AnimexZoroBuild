...
// Always use sequential episode number â€” never raw mal_id (which is a large ID, not 1/2/3)
...
// Scraper modules â€” bundled directly so no separate service is needed
...
exports.getSourcesV2 = async (req, res) => {
  ...
  try {
    ...
    if (title || animeId) {
      ...
      if (mapping && mapping.providers && mapping.providers.length > 0) {
        ...
        for (const providerMatch of validProviders) {
            ...
            console.log(`[V2 Loop] Calling fetchEpisodes for ${providerMatch.providerAnimeId} category: ${category}`);
            const eps = await adapter.fetchEpisodes(providerMatch.providerAnimeId, category);
    ...
    // â”€â”€ HONEST FAILURE (No fakes allowed)
